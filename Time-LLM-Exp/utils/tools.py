...
def log_message(accelerator=None, message='No message!'):
...
class EarlyStopping:
    ...
    def __call__(self, val_loss, model, path):
        ...
        if self.best_score is None:
            ...
            self.counter += 1
            log_message(self.accelerator, f'EarlyStopping counter: {self.counter} out of {self.patience}')
    ...
    def save_checkpoint(self, val_loss, model, path):
        if self.verbose:
            log_message(self.accelerator, f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}). '
                                          f'Saving model ...')
...
def random_replace(embeddings, percentage, min_val=-1, max_val=1):
    num_to_replace = int(embeddings.numel() * percentage / 100)
    indices = torch.randperm(embeddings.numel())[:num_to_replace]
    random_values = torch.FloatTensor(num_to_replace).uniform_(min_val, max_val).to(embeddings.device).to(embeddings.dtype)
    flattened_embeddings = embeddings.view(-1)
    flattened_embeddings[indices] = random_values
    return flattened_embeddings.view(embeddings.shape)
