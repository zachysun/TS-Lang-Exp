...
class Encoder_PCA(nn.Module):
    ...
    def forward(self, x):
        ...
        # self.word_embedding.to(x.device)
        ...
        x, A = self.cross_attention(q, k, v, need_weights=True)
        # print('Shape of Attention Weight:', A.shape)  # (batch_size, num_channels, num_prototypes)
...
class Model(nn.Module):
    def __init__(self, configs, device):
        ...
        # word_embedding = torch.tensor(torch.load(configs.word_embedding_path))
    ...
    def forecast(self, x):
        ...
        # print('time series shape (model input):', x.shape)
        outputs_time1, outputs_text1, Att = self.in_layer(x)

        aligned_textual_token = outputs_text1
        ...
        return {
            ...
            'aligned_textual_token': aligned_textual_token,
        }
